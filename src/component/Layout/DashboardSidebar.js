...
const DashboardSidebar = ({ onMobileClose, openMobile }) => {
	...
	const [sidebarMenu, setSidebarMenu] = useState(CompletedMenu);
	const [userAvatar, setUserAvatar] = useState(null);
	const [userDetail, setUserDetail] = useState(userinitialDetail);

	const handleSidebarItemClick = (itemIndex) => {
		setSidebarMenu(sidebarMenu.map((item, index) => (index === itemIndex ? { ...item, isOpen: !item.isOpen } : item)));
	};

	const handleAllClose = (itemIndex) => {
		setSidebarMenu(
			sidebarMenu.map((item, index) => (index !== itemIndex ? { ...item, isOpen: false } : { ...item, isOpen: !item.isOpen }))
		);
	};
	...
	useEffect(() => {
		if (merchantInformation) {
			const { firstName, lastName } = merchantInformation.data;
			setUserDetail({
				...userDetail,
				firstName,
				lastName,
	...
	useEffect(() => {
		if (merchantInformation) {
			const { approved, merchantLifeCycle } = merchantInformation.data;
			if (approved && merchantLifeCycle === 'Approved') {
	...
	useEffect(() => {
		if (userInformation) {
			const { avatar, firstName, lastName } = userInformation.data;
		...
		>
			...
			>
				<Avatar
					...
					src={userAvatar ? `${avatarUrl}${userAvatar}` : ''}
					style={{
						cursor: 'pointer',
						width: 64,
						height: 64,
					...
					to='/admin/profile'
				/>
				<Typography color='textPrimary' variant='h5' style={{ marginTop: '1rem' }}>
